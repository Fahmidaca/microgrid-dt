...
%   2. Confirm it is <= 6 pages. Current figure set (7): spectrum,
%      anomaly null-control, SHAP, dashboard, bridge confusion, cost
%      sensitivity, Mesa Del Sol - the real-data confusion matrix was
%      dropped as redundant with the 99.7% figure already in text/
%      Table I, and the HOMER architecture table was folded into one
%      sentence in Sec. III-F to make room for the cost-sensitivity
%      figure instead. All figures use [t] (not [H]) so LaTeX can pack
%      them efficiently. If it still overflows, the next cut is
%      shortening Related Work para. 3 (techno-economic) or para. 1
%      (compensation). Figure filenames match their printed Fig. N
%      number in reading order - keep them in sync if you reorder one.
...
predictive management can help \cite{hossain2026bdmicrogrids},
provided their power quality, reliability, and cost are characterized
as rigorously as a conventional grid asset. Renewables remain a small
share of Bangladesh's generation mix today \cite{akash2024bdrenewables},
which is exactly why the characterization problem matters now, before
that share grows.
...
predicting continuous PQ parameters; ours instead classifies discrete
disturbance \emph{type} and adds SHAP explainability -- essential for
trustworthy smart-grid AI \cite{kandula2026xaireview} -- plus a
cyber-resilience layer. Isolation Forest has smart-grid precedent:
\cite{ahmed2019isolationforest} detect covert data-integrity attacks
on smart meters; we apply it to physical disturbance detection
instead, adding a null-result control neither prior work reports.
\cite{osifeko2025aiforecasting} benchmark 12 models on South African
grid data and document data-sparse, developing-region deployment
constraints -- limited local training data, missing measurement
coverage, poor transfer from developed-country models. Bangladesh's
Jamalpur dataset faces the same constraint category, which is why
every accuracy number here carries an explicit uncertainty or
robustness check (bootstrap CIs, a null-result control, sim-to-real
transfer) rather than a bare point estimate.
\cite{akkaya2024pqclassifier} survey ML algorithms for PQ-disturbance
classification; our four-class Random Forest sits within that family
but is additionally validated against physics-simulated, not just
held-out real, waveforms (Section~\ref{sec:bridge}).
...
(THD 1.4\%, \$0.8/Wh), and \cite{araoye2024homergoa} use HOMER paired
with a grasshopper optimization algorithm for rural hybrid-microgrid
sizing -- further evidence HOMER is a standard, not idiosyncratic,
choice for the techno-economic half of this work.
\cite{zhao2025batterydegradation} model energy-storage degradation
cost directly in microgrid energy management, the same problem our
twin's Monte Carlo battery model addresses; we instead keep that
model and HOMER's independent optimizer separate and both
...
(Fig.~\ref{fig:spectrum}). Three representative
operating points drawn from the HOMER annual simulation -- peak load,
sharpest renewable drop, and low-solar/high-wind/high-load -- all
suppress to the same THD level (an expected consequence of the
harmonic model's fixed percentage-of-fundamental structure, reported
as suppression \emph{robustness}), while projected battery cost
varies by an order of magnitude across the same three hours.

\begin{figure}[t]
\centerline{\includegraphics[width=\linewidth]{figs/fig1_spectrum.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\linewidth]{figs/fig2_anomaly_null_control.png}}
...
reaches 99.7\% held-out accuracy (99.26\% mean over five seeds).
SHAP analysis (Fig.~\ref{fig:shap}) shows \texttt{voltage\_rms\_V}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\linewidth]{figs/fig3_shap.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\linewidth]{figs/fig4_dashboard.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\linewidth]{figs/fig5_bridge_confusion.png}}
...
Pro's own optimization shows renewable fraction and cost of energy
strongly inversely related across three candidate architectures:
81.4\%-renewable (PV+WT+FC+BESS) reaches \$0.0278/kWh (NPC \$501{,}600);
53.3\%-renewable (WT+FC+BESS) reaches \$0.0671/kWh (NPC \$736{,}617);
and the 0\%-renewable, fuel-cell-only baseline (FC+BESS) costs
\$0.1032/kWh (NPC \$1{,}120{,}089) -- roughly four times the
81.4\%-renewable configuration's cost of energy.

\begin{figure*}[t!]
\centerline{\includegraphics[width=0.85\textwidth]{figs/fig6_cost_sensitivity.png}}
...
\begin{figure*}[t!]
\centerline{\includegraphics[width=0.85\textwidth]{figs/fig7_mesa_del_sol.png}}
...
\hline
Anomaly detector: ROC-AUC 0.70 vs.\ 0.50 (null) & III-B \\
\hline
...
\hline
Ext.\ V/f check: std $\approx$0.03\% (f), $\approx$0.5--0.6\% (V) p.u. & \ref{sec:external-validation} \\
\hline
\end{tabular}
\end{table}
...
\bibitem{akkaya2024pqclassifier} R. Akkaya, A. V. Y\"{u}ksek, and M.
Akg\"{u}n, ``A comprehensive research of machine learning algorithms
for power quality disturbances classifier based on time-series
window,'' \emph{Electr. Eng.}, 2024, doi:
\href{https://doi.org/10.1007/s00202-023-02177-8}{10.1007/s00202-023-02177-8}.
...
\bibitem{araoye2024homergoa} T. O. Araoye, E. C. Ashigwuike, M. J.
Mbunwe, O. I. Bakinson, and T. I. Ozue, ``Techno-economic modeling and
optimal sizing of autonomous hybrid microgrid renewable energy system
for rural electrification sustainability using HOMER and grasshopper
optimization algorithm,'' \emph{Renew. Energy}, vol. 229, p. 120712,
2024, doi:
\href{https://doi.org/10.1016/j.renene.2024.120712}{10.1016/j.renene.2024.120712}.

\bibitem{zhao2025batterydegradation} Y. Zhao, H. Li, C. Wan, D. Du,
and B. Chen, ``Microgrid energy management considering energy storage
degradation cost,'' \emph{Batteries}, vol. 11, no. 5, p. 169, 2025,
doi: \href{https://doi.org/10.3390/batteries11050169}{10.3390/batteries11050169}.

\bibitem{john2026aipqreview} S. S. John, H. A. Sony, L. V., and
M. P. S., ``Power quality improvement in microgrids using artificial
...
\bibitem{hossain2026bdmicrogrids} M. K. Hossain, A. S. Molla, P.
Chowdhury, F. A. Tanvir, and O. Farrok, ``Transforming sustainable
energy through microgrids: Bangladesh perspectives,'' \emph{Energy
Convers. Manag.: X}, vol. 30, p. 101585, 2026, doi:
\href{https://doi.org/10.1016/j.ecmx.2026.101585}{10.1016/j.ecmx.2026.101585}.

\bibitem{akash2024bdrenewables} F. A. Akash, S. M. Shovon, W. Rahman,
M. A. Rahman, P. Chakraborty, and M. U. Monir, ``Greening the grid: A
comprehensive review of renewable energy in Bangladesh,'' \emph{Heliyon},
vol. 10, no. 5, p. e27477, 2024, doi:
\href{https://doi.org/10.1016/j.heliyon.2024.e27477}{10.1016/j.heliyon.2024.e27477}.
