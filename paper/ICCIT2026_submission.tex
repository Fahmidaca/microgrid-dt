...
%   2. Confirm it is <= 6 pages. Figures were deliberately cut to 6
%      (spectrum, anomaly null-control, SHAP, dashboard, bridge
%      confusion, Mesa Del Sol) - the cost-sensitivity histogram and
%      the real-data confusion matrix were dropped as redundant with
%      numbers already stated in Table I/II and body text. All figures
%      use [t] (not [H]) so LaTeX can pack them efficiently; forcing
%      exact placement previously pushed this to 7 pages with ugly
%      empty/crowded gaps. If it still overflows, shorten Related Work
%      para. 2 next, then fold Table II into a sentence in Sec. III-F.
%      Figure filenames now match their printed Fig. N number in
%      reading order - keep them in sync if you reorder a figure.
...
estimate. On classification specifically, \cite{akkaya2024pqclassifier}
survey ML algorithms for PQ-disturbance classification on time-series
windows; our own four-class Random Forest sits within that same
family but is additionally validated against physics-simulated,
not just held-out real, waveforms (Section~\ref{sec:bridge}).
...
interval of [19{,}623, 34{,}171]. HOMER
...
\hline
\rowcolor{gray!20}
\textbf{Config} & \textbf{RE Frac.} & \textbf{COE (\$/kWh)} & \textbf{NPC (\$)} \\
\hline
...
\begin{figure*}[t!]
\centerline{\includegraphics[width=0.85\textwidth]{figs/fig6_mesa_del_sol.png}}
