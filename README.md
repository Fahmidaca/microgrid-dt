...
Hi. This is my final-year project on building a digital twin for renewable
microgrids. The idea is to combine two kinds of work:
...
I did the ML side (with a lot of help figuring out what to actually
build). My EEE teammate is working on the Simulink model. This README
is my attempt to explain everything I have in this repo so far, in
plain terms.

If you're reading this from my team — please jump straight to the
**"How to run everything"** section. If you're my supervisor — the
...
I built three separate ML pipelines this week:

1. **UCI Grid Stability** (yesterday) — 5 classifiers on a public
   benchmark dataset, plus a new robustness metric I made up called
   **tau-robustness margin**. Best accuracy 96.05%.
2. **Synthetic Bangladesh microgrid dataset** (today, morning) —
   50,000 rows, 50 columns of made-up but physics-informed grid data.
   Used it to train a **3-class IEEE 519 compliance classifier**
   (PASS / MARGINAL / FAIL). Best accuracy 99.82%.
3. **Multi-horizon THD forecasting** (today, afternoon) — LSTM, GRU,
   and MLP predicting voltage THD 5, 15, and 30 minutes ahead. All
   three deep models beat the naive persistence baseline by ~30%
   on RMSE.

Alongside these, my EEE teammate's Simulink script is in `eee_sim/`
and I made a runnable Python port of it that reproduces the headline
result: **THD 27% → 2%** when the Active Power Filter turns on.
...
```bash
# UCI Grid Stability work (yesterday) — total ~5 minutes
...
# Synthetic dataset + downstream pipelines (today)
...
## Part 1 — UCI Grid Stability pipeline (yesterday)
...
It's an old dataset but still cited in 2024 and 2025 papers because
"benchmarks don't expire, methods do." I'm using it as the CSE-side
foundation because it's peer-reviewed, well understood, and lets me
...
operator cares about *safety buffers*, not just correctness. So I made
up a new metric.

For every correctly-classified test point, I do a binary search over
the smallest reaction-time perturbation `||delta_tau||_inf` (in
seconds) that flips the model's prediction. I search all 16 sign
patterns on `tau1..tau4` and clip to the simulation range `[0.5, 10]`.
The result is a **per-sample tau-robustness margin** — how much can
consumer response times degrade before the model's stability call
changes.
...
That's the publishable insight of the CSE-side work.

### Honesty notes for Part 1

- The dataset is static configurations, not time series. I do not claim
  "early warning" prediction on this data because there is no time
  axis to predict along.
- I do not claim the results transfer to real PMU / SCADA data.
...
## Part 2 — Synthetic Bangladesh microgrid dataset (today, morning)

### Why synthetic in the first place

For the second half of the project I wanted a dataset with more
variables than UCI Grid — voltage, current, THD, frequency,
irradiance, battery state, load, and so on — plus a time axis so we
can do forecasting. Real datasets with all that in one place are hard
to get, so I built a synthetic one.
...
that reminds me (and anyone else) that this is for pipeline
development only. When my EEE teammate's Simulink IEEE 14-bus model
starts producing real output, I will swap that in and rerun the same
scripts.

I flagged this to my supervisor because our teacher Faisal Sir was
very clear in his announcement that fabricated data is one of the
worst research-integrity problems. Building a synthetic dataset to
test the ML code is fine; presenting synthetic results as real is not.
...
For the parquet file (which is faster to work with) I use the free
**Tad viewer** (https://www.tadviewer.com — MIT-licensed, no account,
no fees). The VS Code "parquet-viewer" extension by dvirtz also works.

---

## Part 3 — IEEE 519 compliance classifier (today, morning)
...
I use macro-F1 alongside plain accuracy, and I report per-class F1 so
the FAIL class (which is the rarest and most operationally important)
doesn't get lost.

Crucially, I **drop V-THD and I-THD and all six harmonic percentages
from the feature set.** Those are what define the label, so keeping
them would be trivial. The classifier only sees indirect operational
signals — voltage, current, load, storage state, weather. The idea
is to detect impending violations *before* a THD analyzer confirms
them.
...
therefore the compliance verdict. Even after I dropped the direct
THD columns, the model can reconstruct them almost perfectly from the
other inputs.

Real grid data has much more chaos. When my EEE teammate's real
Simulink output replaces the synthetic data, I expect accuracies to
land somewhere in the 85–92% range, which will actually be a much
better story for a paper.
...
## Part 4 — Multi-horizon THD forecasting (today, afternoon)
...
days), so I built a forecasting pipeline that predicts voltage THD
five, fifteen, and thirty minutes ahead using the last thirty minutes
of operational signals as input.
...
information into the training set, which is a common time-series
mistake I wanted to avoid.
...
"will there be a breach?" classification. That's my next step.
...
This part is mostly my teammate's work. I helped clean up the MATLAB
script and wrote a runnable Python port so the whole team can see
results without a MATLAB license.
...
- [`eee_sim/microgrid_pq_twin.py`](eee_sim/microgrid_pq_twin.py)
  — my Python port, runs anywhere with Python 3.10+
...
- **Real data from EEE teammate.** The IEEE 14-bus Simulink model
  producing actual `.slx` outputs. All my synthetic-data results
  need to be re-run on that before the paper submission.
...
- **Statistical significance for the forecasting numbers.** Right
  now I only report point estimates. Multi-seed + bootstrap CI is
  the fix — same treatment as the UCI baselines.
- **Reproducibility audit.** UCI-side has one; the synthetic-data
  side does not yet.
- **Physical validation from the EEE side.** The Simulink builder
  reproduces the expected THD suppression, but a formal
  reproduction of Schafer 2016 is not done.
- **Paper draft.** Structure is in
  [`PAPER_OUTLINE.md`](PAPER_OUTLINE.md); the LaTeX manuscript
...
4. **The bridge** — my trained ML model *is* the "AI-Optimized Control"
   in the EEE compliance table, which is what makes this an
   interdisciplinary paper rather than two separate ones.
...
This is my final-year project at RUET. If you're a teammate or my
supervisor and something in the repo is broken, ping me on WhatsApp.
